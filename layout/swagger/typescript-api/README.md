...
type
UnwrappedApiClient < T > = {
    [K in keyof Api < T > ["api"]
]:
TransformFunction < Api < T > ["api"][K] >;
}
;

type
AdapterAPI < T > = UnwrappedApiClient < T >;

function createNewAPI(): AdapterAPI<unknown> {
    const client = new HttpClient < unknown > ();
    client.instance = (http.constructor
    as
    unknown
    as
    PureHTTP
